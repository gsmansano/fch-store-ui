...
// components
import { useGetCategoryById, useUpdatePartner } from '../../services/useCategoryServices';

export const CategoryEdit = () => {

    const navigate = useNavigate();
    const { doRequest, data: result, isLoading: isEditing } = useUpdatePartner();
    const { data, getData } = useGetCategoryById();
    ...
    const formik = useFormik({
        ...
        onSubmit: async (formData) => {
            await doRequest(categoryId, { name: formData.name, isActive, categoryId });
        }
    });
